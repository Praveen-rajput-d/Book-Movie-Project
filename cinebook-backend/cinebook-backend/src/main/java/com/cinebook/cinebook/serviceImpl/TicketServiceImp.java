...
public class TicketServiceImp  implements TicketService {
    ...
    public TicketResponseDto getTicketById(Long id) {
        ...
        return toTicketDtoWithQr(ticket);
    }
...
//        return ticketList.stream().map(ticketMapper::todto).toList();
        return  ticketList.stream().map(this::toTicketDtoWithQr).toList();
    }
    ...
    public Page<TicketResponseDto> getAllTickets(int page, int size, String sortBy, String sortDir) {
        ...
        return pages.map(this::toTicketDtoWithQr);
    }
    ...
    public List<TicketResponseDto> allTickets() {
        return ticketRepository.findAll().stream().map(this::toTicketDtoWithQr).toList();
    }


    //helper method for qr code genearation for every endpoints
    private TicketResponseDto toTicketDtoWithQr(Ticket ticket){
